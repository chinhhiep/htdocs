...
<?php
	...
	if ($func == "update")
	{
		...
		{
			$db->query("update tgp_cms_menu set ten = '".$db->escape($txt_ten)."', cat = '".$db->escape($txt_cat)."', hien_thi = '".($txt_hien_thi+0)."', type = '".($txt_type+0)."', noi_bat = '".($txt_noi_bat+0)."' where id = '".$id."'");
	...
	if (!$OK)
		template_edit("?act=cms_menu_edit","update",$id,$txt_cat,$txt_ten,$txt_hien_thi,$txt_type,$txt_noi_bat,$error);
?>
</center>